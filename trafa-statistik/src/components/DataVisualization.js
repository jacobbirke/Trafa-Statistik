...
import { fetchData1, fetchData2 } from '../services/dataService';

const DataVisualization = () => {
  const [data1, setData1] = useState([]);
  const [categories1, setCategories1] = useState([]);
  const [statNames1, setStatNames1] = useState({ gtrparb: '', ptrparb: '' });
  ...
  const filterDataByYear = (data, years) => {
    return data.filter(item => years.includes(item.year.toString()));
  };

  const chartOptions = {
    chart: {
      zoomType: 'xy' // Enables zooming on both axes
    },
