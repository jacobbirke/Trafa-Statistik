...
export const handleGenerateChart = (chart:Highcharts.Chart, config:Config, container:HTMLElement ) => {
    const {dimensions, measures, xAxisDimensions, jsonData, seriesDimension, is3D, barMeasure, lineMeasure, chartType, title} = config;
    ...
    if (config.chartType === "pie") {
      ...
      const seriesDim = config.dimensions.find((dim) => dim.name === config.seriesDimension);
      if (!seriesDim) {
      ...
      const measureIndex =
      config.dimensions.length + config.measures.findIndex((m) => m.name === measure.name);
    ...
    if (config.chartType === "stacked") {
      ...
      const selectedMeasuresForStacked = config.measures.filter((m) => m.isSelected);
